...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    #get number of trusted entities and fail compliance if > 1
    no_of_trusted_entities = get_no_of_trusted_entities(iam_client, role_name)
    if no_of_trusted_entities > 1:
        return build_evaluation_from_config_item(configuration_item, "NON_COMPLIANT", annotation='Role "' + role_name + '" has more than one trusted entity')
    return "COMPLIANT"

def get_no_of_trusted_entities(iam_client, role_name):
    if "Service" in iam_client.get_role(RoleName=role_name)['Role']['AssumeRolePolicyDocument']['Statement'][0]['Principal']:
        get_entities_in_assume_role_policy_document = iam_client.get_role(RoleName=role_name)['Role']['AssumeRolePolicyDocument']['Statement'][0]['Principal']['Service']
    else:
        get_entities_in_assume_role_policy_document = iam_client.get_role(RoleName=role_name)['Role']['AssumeRolePolicyDocument']['Statement'][0]['Principal']['AWS']
    print(get_entities_in_assume_role_policy_document)
    entity_delimiter = ","
    no_of_trusted_entities = str(get_entities_in_assume_role_policy_document).count(entity_delimiter) + 1
    print(no_of_trusted_entities)
    return no_of_trusted_entities
